...
router.patch('/read', jwtAuth.auth('user'), async function(req, res, next) {
  ...
  */
  ...
  try {
    const clientId = getClientId(req);
    await notificationModel.updateReadState(clientId, { userId: req.user._id });
    res.json({ ok: 1 });
  }catch(err){
    next(err);
  }
...
// 알림 읽음 처리
router.patch('/:_id/read', async function(req, res, next) {
  ...
  */
  ...
  try {
    ...
    if(item && item.user_id === req.user._id){
      await notificationModel.updateRead(clientId, _id);
      res.json({ ok: 1 });
    }else{
      next();
    }
